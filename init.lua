...
local function extract_location(text, syntax, column, complete, strip_count)
		...
		if not fname then
			local patt
			local cmdpatt = P":" * C(R"09"^1 + "/" * (1 - P"/")^1 * "/")
			if complete then
				local fnamepatt = P(1 - P":")^1
...
local function goto_location(text, win, split)
	...
	if not same or split and same then
		...
		if realname then
			...
			vis:command(string.format("%s %s", (split and "open" or "e"), realname))
		else
...
vis.events.subscribe(vis.events.WIN_OPEN, function(win)
	...
	if M.workspaces and win.file.path and win.syntax then
		...
		for wsdir, map in pairs(M.workspaces) do
			if dirname:match("^" .. wsdir) then
...
vis:map(vis.modes.NORMAL, "<C-o>", function()
	...
	if last.path ~= vis.win.file.path then
		vis:command(string.format("e %s", last.path))
		if last.path ~= vis.win.file.path then
			return false
		end
	end
...
end, "Go to older cursor position in jump list")
